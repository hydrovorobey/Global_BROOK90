...
brook90.framework = function  (catchment_path,
                               ...
                               check_packages_updates = NA) {

  if (isTRUE(check_packages_updates)) {
    package_names = c("raster","elevatr","rgdal","sp","ecmwfr","keyring","lubridate","rgeos","stringr",
  ...
  for (i in 1:length(output_variables)) {
    ...
    if (nrow(hydrotops)==1) {
      result_list_of_df[[i]][,'weighted_average'] = result_list_of_df[[i]][,3]
     } else {
  ...
  for (i in 1:length(output_variables)) {
    ...
    if (ncol(temp_df)==3) {
      temp_df$min = temp_df[,3]
      temp_df$max = temp_df[,3]
    } else {
